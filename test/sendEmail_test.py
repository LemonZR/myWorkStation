...
class emaiMange(object):
    def __init__(self, to=None, cc=None):
        ...
        if cc is None:
            self.listCc = 'zhangrui_miso@si-tech.com.cn'
        else:
            self.listCc = cc.split(",")

    # subject:邮件主题;content:邮件正文;files:附件，list类型
    def moblieSend(self, subject, content, strMsgHtml=None, listImagePath=None, attachment=None):
        retry = 0
        while retry < 3:
            try:
                msg = MIMEMultipart()
                msg['From'] = formataddr([u"流量对账".encode('gbk'), self.smtpuser])
                msg['Subject'] = subject
...
if __name__ == "__main__":
    a = u"mail.py"
    attachment = [a]
    emaiMange(to='zhangrui_miso@si-tech.com.cn', cc='zhangrui_miso@si-tech.com.cn').moblieSend("ces", "test",
                                                                                               attachment)
