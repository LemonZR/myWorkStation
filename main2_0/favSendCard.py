    ...
    def runSend(self,tName,reult_file):
        while True:
            ...
            if  now >= startTime  and now <= endTime :
                try:
                    phone_batch_list =self.phone_que.get(block=False)
                    try:
                        phone_no = phone_batch_list[0]
                        batch_id = phone_batch_list[1]
                        print phone_no + batch_id
                        #result = self.send(phone_no,batch_id)
