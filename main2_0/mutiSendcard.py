...
class NewSendcard(object):
    ...
    def send(self,phone,batchID):
        ...
        # 创建请求生产
        reqSes = self.reqSes
        tName = random.randint(100000, 999999).__str__()
        serialNumber = self.channelId + self.prov_code +tName+self.getMMtime()
    ...
    def _init_phone_queue(self):
        with open('%s.dat' % (self.phone_file), 'r') as csvfile:
            print "self.phone_file"+self.phone_file
    ...
    def mutiRun(self):
        threads=[]
        result_file = open('%s.txt'%self.phone_file,'a')
        ...
        self.logger.error('result file is %s.txt'%self.phone_file)
