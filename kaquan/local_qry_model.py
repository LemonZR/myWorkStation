...
def getConnect():
    try:
        return config.local_mysql,config.local_mysql
...
class kaQ(object):
    ...
    def get_sql(self,sql_format ):
        ...
        for databaseNum in range(1,6):#range(1,6)---> 序号1到5的数据库
            for phoneTail in range(0,10):#号码尾号0-9
                sql=select_from + 'zhangruitest.card_id '
                sqls+=sql+";"
                print sql
        return sqls
    ...
    def mutithread_query(self,sql_format,res_file_name):
        ...
        for conn_data in self.connect_info:
            ...
            thread_id += 1
            thread_list.append(threading.Thread(target=self.qry,args=(sqls,res_file,connectInfo),name=thread_id))
...
if __name__== "__main__":
    ...
    '''table_name 是 分隔符'''
    sql_format = '''select SQL_NO_CACHE * from table_name   '''
