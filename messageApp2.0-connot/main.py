...
logFile = os.sep.join((BASE_DIR, "messageApp2.0", "log", today + ".log"))
...
def generatePhoneDataCmd(filteredDict):
    ...
    for batch, info in filteredDict.items():
        try:
            ...
            obtainEndTime = info["obtainBeginTime"]
            obtainEndDate = myDate.toDate("%Y%m%d", obtainEndTime)
            termType =info["termType"]
            expire_time = '$8'
            lifecycle_st = '$12'
            ...
            while tmp  <= yesterday:
                ...
                day = myDate.toDate("%d", tmp)
                fsPath = hadoop.path('card', '%s', '%s', '%s') % (year, month, day)
...
def scpFileToMsgCt(finalDataFile):
    try:
        ...
        scpSh = os.sep.join((BASE_DIR, "messageApp2.0", "scp", "scp.sh"))
...
if __name__ == "__main__":
    ...
    qryDate = yesterday
    fileDir = os.sep.join((BASE_DIR, "messageApp2.0", "messageFile", today))
    mkdir(fileDir)
    ...
    finalDataFile = os.sep.join((BASE_DIR, "messageApp2.0", "messageFile", today, "2_%s01.txt" % timexx))
